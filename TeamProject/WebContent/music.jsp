	...
	<nav class="navbar navbar-default">
		...
		<div class="collapse navbar-collapse"
			...
			<ul class="nav navbar-nav">
				...
				<li class="active"><a href="music.jsp"><i class="fas fa-music"></i>&nbsp;Music</a></li>
			</ul>
			<%
				if(userID == null){
			%>
		...
		<div class="row">
			<table class="table table-hover"
				...
				<tbody>
					...
					ArrayList<Bbs> listmusic = bbsDAO.getListmusic(pageNumber);
					for(int i = 0; i < listmusic.size(); i++) {
						
					%>
					<tr>
						<td><%= listmusic.get(i).getNum() %></td>
						<td><a href="view.jsp?num=<%= listmusic.get(i).getNum() %>"><%= listmusic.get(i).getTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt").replaceAll("\n", "<br>") %></a></td>
						<td><%= listmusic.get(i).getUserID() %></td>
						<td><%= listmusic.get(i).getCtg() %></td>
						<td><%= listmusic.get(i).getBbsDate().substring(0, 11) + listmusic.get(i).getBbsDate().substring(11, 13) + "시" + listmusic.get(i).getBbsDate().substring(14, 16) + "분" %></td>
					</tr>
					<%
						}
					%>
