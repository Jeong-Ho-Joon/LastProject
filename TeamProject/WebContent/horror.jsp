	...
	<nav class="navbar navbar-default">
		...
		<div class="collapse navbar-collapse"
			...
			<ul class="nav navbar-nav">
				...
				<li class="active"><a href="horror.jsp"><i class="fas fa-ghost"></i>&nbsp;Horror</a></li>
		...
		<div class="row">
			<table class="table table-hover"
				...
				<tbody>
					...
					ArrayList<Bbs> listhorror = bbsDAO.getListhorror(pageNumber);
					for(int i = 0; i < listhorror.size(); i++) {
						
					%>
					<tr>
						<td><%= listhorror.get(i).getNum() %></td>
						<td><a href="view.jsp?num=<%= listhorror.get(i).getNum() %>"><%= listhorror.get(i).getTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt").replaceAll("\n", "<br>") %></a></td>
						<td><%= listhorror.get(i).getUserID() %></td>
						<td><%= listhorror.get(i).getCtg() %></td>
						<td><%= listhorror.get(i).getBbsDate().substring(0, 11) + listhorror.get(i).getBbsDate().substring(11, 13) + "시" + listhorror.get(i).getBbsDate().substring(14, 16) + "분" %></td>
					</tr>
					<%
						}
					%>
