	...
	<nav class="navbar navbar-default">
		...
		<div class="collapse navbar-collapse"
			...
			<ul class="nav navbar-nav">
				...
				<li class="active"><a href="humor.jsp"><i class="fas fa-grin-squint"></i>&nbsp;Humor</a></li>
		...
		<div class="row">
			<table class="table table-hover"
				...
				<tbody>
					...
					ArrayList<Bbs> listhumor = bbsDAO.getListhumor(pageNumber);
					for(int i = 0; i < listhumor.size(); i++) {
						
					%>
					<tr>
						<td><%= listhumor.get(i).getNum() %></td>
						<td><a href="view.jsp?num=<%= listhumor.get(i).getNum() %>"><%= listhumor.get(i).getTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt").replaceAll("\n", "<br>") %></a></td>
						<td><%= listhumor.get(i).getUserID() %></td>
						<td><%= listhumor.get(i).getCtg() %></td>
						<td><%= listhumor.get(i).getBbsDate().substring(0, 11) + listhumor.get(i).getBbsDate().substring(11, 13) + "시" + listhumor.get(i).getBbsDate().substring(14, 16) + "분" %></td>
					</tr>
					<%
						}
					%>
